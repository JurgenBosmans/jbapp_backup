class MeterstandenController < ApplicationController
  ...
  def show
	...
	def winter
		...
		@jaar3 = @jaar - 3
		@jaar4 = @jaar - 4
		@waarde_jaar = Meterstand.where(created_at: Time.new(@jaar1,11,1)..Time.new(@jaar,5,1)).sum("verbruik")
		@waarde_jaar1 = Meterstand.where(created_at: Time.new(@jaar2,11,1)..Time.new(@jaar1,5,1)).sum("verbruik")
		@waarde_jaar2 = Meterstand.where(created_at: Time.new(@jaar3,11,1)..Time.new(@jaar2,5,1)).sum("verbruik")
		@waarde_jaar3= Meterstand.where(created_at: Time.new(@jaar4,11,1)..Time.new(@jaar3,5,1)).sum("verbruik")
