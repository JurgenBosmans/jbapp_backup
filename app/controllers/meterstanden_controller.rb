class MeterstandenController < ApplicationController
  ...
  def show
	...
	def winter
		...
		@jaar3 = @jaar - 3
		@jaar4 = @jaar - 4
		@waarde_jaar = Meterstand.where(datum: Time.new(@jaar1,11,1)..Time.new(@jaar,4,30)).sum("verbruik")
		@waarde_jaar1 = Meterstand.where(datum: Time.new(@jaar2,11,1)..Time.new(@jaar1,4,30)).sum("verbruik")
		@waarde_jaar2 = Meterstand.where(datum: Time.new(@jaar3,11,1)..Time.new(@jaar2,4,30)).sum("verbruik")
		@waarde_jaar3= Meterstand.where(datum: Time.new(@jaar4,11,1)..Time.new(@jaar3,4,30)).sum("verbruik")
